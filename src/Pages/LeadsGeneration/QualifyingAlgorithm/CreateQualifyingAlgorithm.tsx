...
import { CreateQualifyingAlgorithm as CreateQualifyingAlgorithmParams } from "@/Interfaces/LeadsGeneration/qualifying-algorithm.interface";
...
export function CreateQualifyingAlgorithm({ isOpen, setIsOpen }: Props) {
  const queryClient = useQueryClient();
  const { create } = useQualifyingAlgorithmApi();
  ...
  const createQualifyingAlgorithm = useMutation({
    mutationFn: (params: CreateQualifyingAlgorithmParams) => create(params),
  });

  const handleCreateQualifyingAlgorithm = () => {
    if (name === "") {
      showNotification({
        title: "Error",
    ...
    createQualifyingAlgorithm.mutate(
      ...
      },
      {
        onSuccess: () => {
          ...
          showNotification({
            message: "Qualifying algorithm created successfully",
            color: "teal",
            icon: <IconCheck />,
          });
          handleClose();
  ...
  return (
    <Modal
      size="lg"
      opened={isOpen}
      closeOnEscape={!createQualifyingAlgorithm.isPending}
      closeOnClickOutside={!createQualifyingAlgorithm.isPending}
      onClose={handleClose}
      title="Create new qualifying algorithm"
    >
      <Stack>
        <TextInput
          value={name}
        ...
        <Group justify="center">
          <Button
            onClick={handleCreateQualifyingAlgorithm}
            loading={createQualifyingAlgorithm.isPending}
          >
            Save
          </Button>
        </Group>
